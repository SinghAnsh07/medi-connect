...
import { Bell, Home, CalendarDays, MessageSquare, Search, Video, User, LogOut, Sun, Moon, Menu, X } from 'lucide-react';
...
const ClientDashboardNavbar = () => {
    ...
    const navLinks = [
        { to: '/clientdashboard', label: 'Dashboard', icon: Home },
        { to: '/bookappointment', label: 'Appointments', icon: CalendarDays },
        { to: '/chat', label: 'Messages', icon: MessageSquare },
        { to: '/finddoctors', label: 'Find Doctors', icon: Search },
    ...
    return (
        <nav className={`fixed top-0 w-full z-50 border-b backdrop-blur-xl ${isDark ? 'bg-slate-900/95 border-slate-700 text-slate-100' : 'bg-white/95 border-slate-200 text-slate-900'}`}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="h-18 min-h-[72px] flex items-center justify-between gap-4">
                    <Link to="/clientdashboard" className="flex items-center gap-2">
                        <div className={`h-9 w-9 rounded-xl flex items-center justify-center ${isDark ? 'bg-emerald-500/20' : 'bg-emerald-50'}`}>
                            <User className={`h-5 w-5 ${isDark ? 'text-emerald-300' : 'text-emerald-700'}`} />
                        </div>
                        <div className="leading-tight">
                            <div className="text-base font-semibold tracking-tight">MediConnect</div>
                            <div className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Client Portal</div>
                        </div>
                    </Link>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            className={`lg:hidden p-2 rounded-lg border transition-colors ${isDark ? 'border-slate-600 hover:bg-slate-800 text-slate-200' : 'border-slate-200 hover:bg-slate-100 text-slate-700'}`}
            ...
            {isOpen && (
                <div className={`lg:hidden border-t ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-200'}`}>

...
const registerClient = asyncHandler(async (req, res) => {
  ...
  // Send SMS OTP (non-blocking — don't crash registration if Twilio fails)
  sendOtp(phone, otp).catch((err) => {
    console.warn("⚠️ SMS OTP failed (non-fatal):", err.message);
  });
  ...
  try {
    await transporter.sendMail({
      ...
      to: email,
      subject: "Your OTP for MediConnect Verification",
      html: `<p>Your OTP is: <strong>${otp}</strong></p><p>This OTP is valid for 5 minutes.</p>`,
    });
  } catch (mailErr) {
    console.warn("⚠️ Email OTP failed (non-fatal):", mailErr.message);
  }
